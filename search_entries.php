<?php    
    ...
    function search_entries_table()
    {
        ...
        return <<<EOD
            <div class="d-flex flex-column gap-3">
                <div style="max-height: 500px; max-width: 1700px; overflow: auto;">
                    <table class="table table-bordered table-hover" style="width: 1700px;">
                        <thead>
                            <tr>
                                <th>
                                </th>
                ...
                <div class="position-relative d-none" id="table-buttons">
            ...
            <script>            
                ...
                async function fillTable(sort_category, asc) {
                    order = asc ? 'ASC' : 'DESC';
                    ...
                    await fetch('$site_url/wp-admin/admin-ajax.php?action=table_response&applicants-table='+ tableVal + '&address=' + addressVal + '&town=' + townVal + '&applicant-email=' + applicant_emailVal + '&phone=' + phoneVal + '&applicant-name=' + applicant_nameVal + '&c_status=' + cStatusVal + '&w_status=' + wStatusVal + '&slot=' + slotStatusVal + '&time=' + timeVal + '&category=' + category + '&order=' + order)
                        ...
                        .then(entries => {
                            ...
                            entries.forEach(entry => {
                                ...
                                var c_status;
                                if (entry.confirmation_status == 0) {
                                    c_status = "No";
                                }
                                ...
                                table_row.innerHTML = `
                                    <td>
                                        <div class="border border-dark rounded border-5 d-flex justify-content-center" onclick="selectRow(this)" style="width: 43px; height: 43px;">
                                    ...
                                    <td> <input onchange="editEntry('day', 'slot', this)" class="border-0 w-50" type='text' value='` + slot + `' /> </td>
        ...
        Congratulations! You won the Maplewoodstock Canopy Lottery for a canopy location on {day}! If you entered the lottery for multiple days, you will receive a separate notification for each day of the festival. Please read this entire email as action is required to confirm your attendance at the event.
        ...
        PLEASE COMPLETE THIS CONFIRMATION FORM NO LATER THAN JULY 5, 2023 TO CONFIRM YOUR ACCEPTANCE OF THE CANOPY LOCATION AND THE LOTTERY RULES:
        ...
        • Please consider consolidating with another lottery winner as there were hundreds of entrants to the lottery, many from the same streets:
        ...
        We look forward to seeing you at Maplewoodstock!`, 
        `Dear Maplewoodstock Canopy Lottery Entrant,
        ...
        PLEASE COMPLETE THE WAITLIST CONFIRMATION FORM NO LATER THAN JULY 5, 2022 TO CONFIRM YOUR ACCEPTANCE OF THE WAITLIST OPPORTUNITY AND THE LOTTERY RULES:
        
        
        {link_to_confirmation_form}
        
        
        Failure to respond by July 5th will forfeit your waitlist opportunity.
        ...
        IN THE EVENT YOUR PLANS CHANGE AND YOU WON’T BE ABLE TO USE YOUR CANOPY SPOT, PLEASE EMAIL US AT MAPLEWOODSTOCKCANOPIES@GMAIL.COM OR CALL (973) 996-8198 AS SOON AS POSSIBLE.`, 
        `Everyone,

        Thank you so much for participating in the Maplewoodstock Canopy Lottery.  Unfortunately, you did not win a canopy location for {day}.  You will receive a separate notice for the other day if you entered the lottery for both days of the festival.
        
        Hundreds of Maplewood and South Orange residences submitted entries, and we did our best to accommodate as many families as possible.  To this end, we maximized the number of canopy locations possible at the festival site and also ensured no entrant won twice while others did not win at all.
        
        
        We strongly encourage everyone to speak to their neighbors - it's likely you know someone who won, and sharing canopy space is the Maplewoodstock way!
        ...
        `MAPLEWOODSTOCK CANOPY UPDATE - You’re off the waitlist and have a confirmed spot for {day}! Reply YES asap to confirm and we’ll email you details. Thanks!!!`,
        `Dear {name},
        ...
        The Maplewoodstock Canopy Team`,
            ``];
                ...
                document.getElementById("submit-filter").addEventListener("click", () => fillTable(prefix + 'canopy_' + table.value + '.slot', true)); 
    ...
    function get_rows($name, $address, $town, $email, $phone, $order, $category, $applicants_table, $slot, $c_status, $w_status, $time, $have_ids)
    {
        global $wpdb;
        ...
        $name_query = $name != "" ? " ".$app_table.".name='$name' AND" : " 1=1 AND";
        $address_query = $address != "" ? " ".$app_table.".address='$address' AND" : " 1=1 AND";
        $town_query = $town != "" ? " ".$app_table.".town='$town' AND" : " 1=1 AND";
        $email_query = $email != "" ? " ".$app_table.".email='$email' AND" : " 1=1 AND";
        $phone_query = $phone != "" ? " ".$app_table.".phone='$phone' AND" : " 1=1 AND";
        $slot_query = $slot != "" ? " ".$day_table.".slot=$slot AND" : " 1=1 AND";
        $c_status_query = $c_status != "" ? " ".$day_table.".confirmation_status=$c_status AND" : " 1=1 AND";
        $w_status_query = $w_status != "" ? " ".$day_table.".win_status='$w_status' AND" : " 1=1 AND";
        $time_query = $time != "" ? " ".$day_table.".arrival_time='$time'" : " 1=1";

        $id_select = $have_ids ? ", ".$day_table.".applicant_id, ".$day_table.".id" : "";

        $entries = $wpdb->get_results("SELECT ".$day_table.".slot, ".$app_table.".name, ".$app_table.".address, ".$app_table.".email, ".$app_table.".town, ".$app_table.".phone, ".$day_table.".confirmation_status, ".$day_table.".arrival_time, ".$day_table.".win_status".$id_select." FROM ".$day_table." INNER JOIN ".$app_table." ON ".$day_table.".applicant_id=".$app_table.".id WHERE".$name_query.$address_query.$town_query.$email_query.$phone_query.$slot_query.$c_status_query.$w_status_query.$time_query." ORDER BY ".$category." ".$order);
    ...
    {
        $entries = get_rows($_GET['applicant-name'], $_GET['address'], $_GET['town'], $_GET['applicant-email'], $_GET['phone'], $_GET['order'], $_GET['category'], $_GET['applicants-table'], $_GET['slot'], $_GET['c_status'], $_GET['w_status'], $_GET['time'], true);
    ...
    function apply_changes() 
    {
        ...
        foreach($updates as $update)
        {
            ...
            $table = $update->table;
            $col = $update->col;
            $value = in_array($col, ['name', 'address', 'town', 'email', 'phone', 'win_status', 'arrival_time']) ? "'".$update->value."'" : $update->value;
    ...
    function spreadsheet()
    {
        ...
        $rows = get_rows($_GET['applicant-name'], $_GET['address'], $_GET['town'], $_GET['applicant-email'], $_GET['phone'], $_GET['order'], $_GET['category'], $_GET['applicants-table'], $_GET['slot'], $_GET['c_status'], $_GET['w_status'], $_GET['time'], false);
        ...
        foreach($rows as $row)
        {
            ...
            else if (is_int($row->confirmation_status) && $row->confirmation_status == 0)
            {
                $c_status = "No";
            }
            else 
            {
                $c_status = $row->confirmation_status;
            }
            array_push($arr, $row->slot, $row->name, $row->address, $row->town, $row->email, $row->phone, $row->win_status, $row->arrival_time, $c_status);
    ...
    function send_message()
    {
        ...
        foreach($ids as $id)
        {
            ...
            $sEMessage = $eMessage;
            $day = $tDay == "saturday" ? "Saturday, 7/8/23" : "Sunday, 7/9/23";
